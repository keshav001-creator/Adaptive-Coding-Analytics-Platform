...
async function logIntelligence(question) {
    ...
    try {
        ...
        for (let i = 1; i < historyOfQuestion.length; i++) {
            ...
            // Calculate improvement percentage and failed attempts data
            const improvementPercentage = ((previousquestion - currentQuestion) / previousquestion) * 100;
            const failedAttemptsData = PreviousFailedAttempts - currentFailedAttempts;


            trends.push({ questionName, improvementPercentage, failedAttemptsData });
        ...
        for (let trend of trends) {
            ...
            if (trend.improvementPercentage > 40) {
                behaviourSummary.push(
                    "Significant improvement in time taken to solve the question."
                );

            } else if (trend.improvementPercentage > 20) {
                behaviourSummary.push(
                    "Moderate improvement in time taken to solve the question."
                );

            } else if (trend.improvementPercentage > 5) {
                behaviourSummary.push(
                    "Minor improvement in time taken to solve the question."
                );

            } else if (trend.improvementPercentage >= 0) {
                behaviourSummary.push(
                    "No significant improvement in time taken to solve the question."
                );

            } else {
                behaviourSummary.push(
                    "Increase in time taken to solve the question."
                );
            }
